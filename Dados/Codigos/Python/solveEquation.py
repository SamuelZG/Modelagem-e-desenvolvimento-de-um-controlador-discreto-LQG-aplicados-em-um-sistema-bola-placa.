...
#values in cm
dm = 1.74 #  distance from motor shaft to pole
dc = 10 #   distance between center pole and side pole
K = dc/dm # factor
...
maxAlpha = int(7.5*10)	# here is the maximum angle alpha in degrees x10 to scale it
...
for alpha in range(-maxAlpha,maxAlpha+1):
    ...
    elif theta > 120 and theta < 180:					# different areas of non linearities
            theta = translate(theta,160,180,150,159)
    #theta = round(degrees(theta),0)
    separator = "|"
    data = str(round((alpha),3)) + separator + str(int(round(theta,0))) + "\n"
    file.write(data)
